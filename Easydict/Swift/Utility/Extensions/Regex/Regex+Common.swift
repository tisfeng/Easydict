...
extension Regex where Output == Substring {
    ...
    ///
    /// **Examples:**
    /// - `"1. First item"` ✓ (English list with space)
    ...
    ///
    /// **Pattern Logic:**
    /// - Matches at line start (with optional leading whitespace)
    /// - Numeric lists: digits followed by dot and (whitespace OR CJK character)
    /// - Letter lists: single letter followed by dot and whitespace
    /// - Roman numeral lists: roman numerals followed by dot and whitespace
    /// - Must have whitespace or CJK character after dot to distinguish from decimals
    ///
    /// **Original regex equivalent:** `^\\s*(\\d+|[a-zA-Z]|[ivxIVX]+)\\.(\\s+|[\\u4E00-\\u9FFF])`
    static var listWithDotPattern: Self {
        Regex {
            ...
            ChoiceOf {
                // Numeric lists with either whitespace or CJK characters after dot
                Regex {
                    OneOrMore(.digit)
                    "."
                    ChoiceOf {
                        OneOrMore(CharacterClass.horizontalWhitespace) // 1. item (with space)
                        One(CharacterClass.cjkChars) // 1.项目 (CJK immediately after dot)
                    }
                }
                // Letter lists with whitespace after dot
                Regex {
                    One(CharacterClass.asciiLetters)
                    "."
                    OneOrMore(CharacterClass.horizontalWhitespace)
                }
                // Roman numeral lists with whitespace after dot
                Regex {
