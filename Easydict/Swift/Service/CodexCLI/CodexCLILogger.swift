...
/// Writes a structured log file for one `codex exec` invocation.
final class CodexCLILogger: @unchecked Sendable {
    // MARK: Lifecycle
    ...
    func finish(stderr: String, exitCode: Int, duration: TimeInterval) {
        ...
        """
        write(footer + "\n")
        CodexCLIDebugLogger.shared.post(footer)
        pruneOldLogs()
    }
    ...
    private let queue = DispatchQueue(label: "codex-cli-logger", qos: .utility)

    private lazy var fileURL: URL? = {
        let pathManager = AppPathManager.current
        let logDir = pathManager.codexCLILogDirectory
...
final class CodexCLIDebugLogger {
    static let shared = CodexCLIDebugLogger()

    static let didAppendNotification = Notification.Name("CodexCLIDebugLogDidAppend")
