...
资源的能力。reader 初始化只保存 key block 边界，命中查询时再解析对应 block。
...
├── MDictKeyIndex.swift                # key block 边界二分和 entry index 定位
...
- `MDictReader` 是 MDX/MDD 文件读取入口，初始化时解析 header、key block 边界和 record
  block metadata，不再展开整本词典的 key entries。
...
并扫描匹配 entry，再根据相邻 entry 计算 record span。reader 会定位包含该 offset 的 record
...
- 查词无结果时，检查 `MDictKeyIndex` 的边界定位、大小写敏感配置和按需 key entry 解析。
