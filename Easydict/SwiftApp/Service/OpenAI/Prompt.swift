...
extension QueryService {
    static let translationSystemPrompt = "You are a translation expert proficient in various languages that can only translate text and cannot interpret it. You are able to accurately understand the meaning of proper nouns, idioms, metaphors, allusions or other obscure words in sentences and translate them into appropriate words by combining the context and language environment. The result of the translation should be natural and fluent, you can only return the translated text, do not show redundant quotes and additional notes in translation."

    func translationPrompt(text: String, from sourceLanguage: Language, to targetLanguage: Language) -> String {
        let prompt = "Translate the following \(sourceLanguage) text into \(targetLanguage) text:\n\n\"\"\"\n\(text)\n\"\"\" "
        return prompt
    }
    ...
    func sentenceMessages(sentence: String, from sourceLanguage: Language, to targetLanguage: Language) -> [[String: String]] {
        ...
        let directTranslationPrompt = "First, translate the sentence into \(targetLanguage) text literally, keep the original format, and don’t miss any information, desired display format: \"\(literalTranslation):\n{literal_translation_result} \",\n\n"
    ...
    func dictMessages(word: String, sourceLanguage: Language, targetLanguage: Language) -> [[String: String]] {
        ...
        let pronunciationPrompt = "Look up its pronunciation, desired display format: \"\(pronunciation): / {pronunciation} /\" \n"
        ...
        \nLook up its brief <\(answerLanguage)> explanation in clear and understandable way, desired display format: "\(explanation): {brief_explanation} "

        """
        prompt += explanationPrompt

        let etymologyPrompt = """
        Look up its detailed \(etymology), including but not limited to the original origin of the word, how the word's meaning has changed, and the current common meaning. desired display format: "\(etymology): {detailed_etymology} " .
        ...
        if isEnglishWord {
            ...
            Look up disassembly and association methods to remember it, desired display format: "\(howToRemember): {how_to_remember} "

            """
            prompt += rememberWordPrompt

            let cognatesPrompt = """
            \nLook up main <\(sourceLanguage)> words with the same root word as "\(word)", no more than 4, excluding phrases, display all parts of speech and meanings of the same root word, pos always displays its English abbreviation. If there are words with the same root, show format: "\(cognate): {cognates} ", otherwise don't display it.
        ...
        if isWord || isEnglishPhrase {
            let synonymsPrompt = """
            \nLook up its main <\(sourceLanguage)> near synonyms, no more than 3, If it has synonyms, show format: "\(synonym): {synonyms} "
            """
            prompt += synonymsPrompt

            let antonymsPrompt = """
            \nLook up its main <\(sourceLanguage)> near antonyms, no more than 3, If it has antonyms, show format: "\(antonym): {antonyms} "

            """
            prompt += antonymsPrompt

            let phrasePrompt = """
            \nLook up its main <\(sourceLanguage)> phrases, no more than 3, If it has phrases, show format: "\(commonPhrases): {phrases} "
...
extension QueryService {
    ...
    func chatMessages(queryType: EZQueryTextType, text: String, from: Language, to: Language) -> [ChatCompletionMessageParam] {
        typealias Role = ChatCompletionMessageParam.Role
