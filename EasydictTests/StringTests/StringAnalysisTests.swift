...
struct StringAnalysisTests {
    ...
    func languageDetection() {
        ...
        #expect("你好".detectLanguage() == .simplifiedChinese, "Chinese text should be detected as Chinese")
        #expect("こんにちは".detectLanguage() == .japanese, "Japanese text should be detected as Japanese")
    ...
    func wordCount() {
        ...
        #expect("The quick brown fox".wordCount == 4, "Sentence with multiple words should be counted correctly")
    ...
    func sentenceCount() {
        #expect("Hello world. How are you?".sentenceCount == 2, "Multiple sentences should be counted correctly")
        #expect(
        ...
        #expect("SingleSentence".sentenceCount == 1, "Single word without punctuation should return 1")
    }
    ...
    func dictionaryQueryDetection() {
        #expect(
            "hello".queryType(language: "en", maxWordCount: 3).contains(.dictionary),
            "Single English word within limit should query dictionary"
        )
        #expect(
            "world".queryType(language: "en", maxWordCount: 1).contains(.dictionary),
            "Single English word at limit should query dictionary"
        )
        #expect(
            "hel".queryType(language: "en", maxWordCount: 2).contains(.dictionary),
            "Two-letter word within limit should query dictionary"
        )
        #expect(
            !"hello world".queryType(language: "en", maxWordCount: 3).contains(.dictionary),
            "Multiple words should not query dictionary"
        )
        #expect(
            !"supercalifragilisticexpialidocious".queryType(language: "en", maxWordCount: 20).contains(.dictionary),
            "Word exceeding max length should not query dictionary"
        )

        #expect(
            "你好".queryType(language: "zh-Hans", maxWordCount: 5).contains(.dictionary),
            "Chinese word within limit should query dictionary"
        )
        #expect(
            "世界".queryType(language: "zh-Hans", maxWordCount: 4).contains(.dictionary),
            "Chinese word at limit should query dictionary"
        )
        #expect(
            !"你好世界".queryType(language: "zh-Hans", maxWordCount: 5).contains(.dictionary),
    ...
    func sentenceQueryDetection() {
        #expect(
            "Hello world, how are you today?".queryType(language: "en", maxWordCount: 1).contains(.sentence),
            "English sentence should query sentence"
        )
        #expect(
            "This is a test".queryType(language: "en", maxWordCount: 1).contains(.sentence),
            "Simple sentence should query sentence"
        )
        #expect(
            "SingleWord".queryType(language: "en", maxWordCount: 1).contains(.sentence),
            "Single word without punctuation should not query sentence"
        )

        // Chinese text should not query sentence even if it's a sentence
        #expect(
            !"你好世界".queryType(language: "zh-Hans", maxWordCount: 1).contains(.sentence),
    ...
    func spellingSuggestions() {
        ...
        #expect(suggestions?.contains("hello"), "Should suggest correct spelling")
        #expect(suggestions?.contains("hell"), "Should suggest alternative spellings")
