...
- `CodexCLIRunnerTests.swift` 是核心测试套件，覆盖多类逻辑：
  - `buildArguments` 必含 `exec --json --skip-git-repo-check --ephemeral
    ...
    且通过 `-- -` 固定从 stdin 读取 prompt，避免长文本进入 argv。
  - `writePromptToStandardInput` 验证 Unicode、前后空白和长文本会原样写入
    stdin pipe。
  - `processCodexStdoutLine` / `terminalAgentMessage` 验证多条
    `agent_message` 只缓存最后一条，且失败或取消时不会输出缓存文本。
  - `parseCodexError` 同时验证 stderr 兜底（`not signed in`、
    `rate limit`、未知错误）和 stdout JSONL 路径（`turn.failed.error`
    既可能是字符串也可能是 `{message:...}` 对象）。
  - `terminalError` 验证用户取消优先于 CLI 退出错误，并保留正常
    成功/失败退出的原有分类。
...
- `CodexCLIServiceTests.swift` 验证服务的元信息：`serviceType()`、
  ...
  `none`，并确认官方 `model_reasoning_effort` 值会原样传给 CLI。
...
- 服务在设置页消失或注册失败 → 看 service tests 的
  `factoryRegistration`；推理等级选项异常 → 看 service tests 的
