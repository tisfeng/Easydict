...
- `CodexCLIRunnerTests.swift` 是核心测试套件，覆盖三类逻辑：
  - `buildArguments` 必含 `exec --json --skip-git-repo-check --ephemeral
    --sandbox read-only -C <tmp> --` 这一固定形状，且 prompt 始终位于
    `--` 终止符之后，防止以 `-` 开头的输入被误判成 flag。
  ...
  - 工具方法 `runWhich`、`resolveLoginShellPath`、`extractCodexText`
    覆盖二进制探测和 shell 路径回退。
