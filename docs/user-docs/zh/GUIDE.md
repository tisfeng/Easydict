...
先为“翻译并替换”和“润色并替换”设置全局快捷键，再前往
**设置 → 高级设置 → 翻译与润色替换** 配置这两个动作。翻译默认使用 BuiltInAI，
润色默认使用 Polishing。

两个动作可以分别使用内置 AI 或已配置的 OpenAI 兼容服务：OpenAI、Custom OpenAI、
DeepSeek、Groq、Zhipu、MiniMax 或 GitHub Models。动作始终沿用“服务”页面中该实例
当前的模型、接口地址、API Key、temperature 和 streaming 设置；多个 Custom OpenAI
实例会保持独立。

“追加要求”是可空的附加指令，不会覆盖默认翻译或润色任务；留空时完全保持默认
Prompt。模型响应仍会按 chunk 逐步替换选中文字。所选服务在产生文字前失败时，
Easydict 只会使用该动作的内置默认服务重试一次；已经开始替换后若响应中断，则保留
部分结果并提示内容可能不完整。

> [!IMPORTANT]
> 选中的文字和追加要求会发送至所选在线服务。
