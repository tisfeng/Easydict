...
- 生产实现与测试可独立推进时可以分工，但同一 workspace 不并发运行 Xcode 构建或测试；
  Git index 和本地交付始终串行。
- 委派时传递目标、有效授权、允许路径、行为预期、冻结实现快照和验证范围。reviewer 只读，
  tester 只修改明确分配的测试与 fixture，不修改生产代码、工程配置、history 或 Git 状态。
- tester 返回测试目的、修改路径、实际命令、结果和阻塞证据；生产缺陷交回主 Agent。
- 主 Agent 核验 finding，在已有授权内修复真实问题；无依据或超范围建议说明原因。实现变化后
  按风险增量复核，最终采用的审查和验证必须覆盖最终快照。
- 有效阻塞 finding、失败验证或必要证据缺失时不能声称完成或自动提交。无法委派时按
...
必要构建或针对性测试。纯治理 Markdown、子代理配置或文档合并只运行静态检查；PR review
能否运行构建以根入口和请求边界为准。
