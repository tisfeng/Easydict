...
- 写入 `.agents/skills/` 或 `.codex/agents/` 前先检查 `skills-lock.json` 与
  `.codex/agents-lock.json`。受管路径只允许在明确获准的上游同步任务中由对应安装器更新，
  不允许项目内直接编辑；详细边界见
  [`external-agent-assets.md`](external-agent-assets.md)。
...
1. 用户是否明确授权本次类型的写入？
2. 目标结果和允许修改的路径是否明确？
3. 目标是否是 lock 管理的外部资产；若是，当前任务是否明确授权从正确上游同步？
4. 初始 Git 状态是否允许安全区分 Agent 变更与用户变更？
5. 是否已确定必要的 history，以及多步骤或高风险工作所需的 active plan？
6. 完成后要运行哪些检查，哪些检查无法运行？
